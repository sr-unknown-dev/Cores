...
class ClaimManager
{
    ...
    {
        # Register handler
        Loader::getInstance()->getServer()->getPluginManager()->registerEvents(new ClaimListener(), Loader::getInstance());
        foreach (Loader::getInstance()->getProvider()->getClaims() as $name => $data)
            $this->createClaim($name, $data['type'], (int) $data['minX'], (int)$data['maxX'], (int)$data['minZ'], (int)$data['maxZ'], $data['world']);
