        ...
        if ($player instanceof Player)
            if ($item->getNamedTag()->getTag("Abilities") !== null) {
                if ($item->getNamedTag()->getString("Abilities") === "Strength") {
                    $event->cancel();
                    if ($player->getSession()->getCooldown('ability.Strength') === null) {
                        if ($player->getSession()->getCooldown('ability.global') === null) {
                            ...
                            $player->sendMessage("§6You have activated §6Strength");
                            $player->getSession()->addCooldown('ability.Strength', ' §l§7×§r §6Strength§r§f: §f', 45);
                        ...
                        $player->sendMessage("§cNo puedes usar el §6Strength§c por " . Timer::format($player->getSession()->getCooldown("ability.Strength")->getTime()));
                    }
                }
            }
    }
