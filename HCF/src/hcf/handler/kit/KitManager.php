...
class KitManager
{
    ...
    public function __construct(){
        # Register kits
        foreach (Loader::getInstance()->getProvider()->getKits() as $name => $data) {
        ...
        ClassFactory::init();
        Loader::getInstance()->getServer()->getPluginManager()->registerEvents(new KitListener(), Loader::getInstance());
    }
    ...
    {
        return Loader::getInstance()->getProvider()->getKitConfig()->get('organization');
    }
    ...
    public function addKit(string $kitName, string $nameFormat, ?string $permission, ?Item $itemRepresentative, array $items, array $armor, int $cooldown, bool $new = true): void
    {
        ...
        if ($new) {
            ...
            Loader::getInstance()->getProvider()->getKitConfig()->set('organization', $organization);
            Loader::getInstance()->getProvider()->getKitConfig()->save();
        }
    }
