...
class EnderpearlEntity extends Throwable
{
	...
	protected float $gravity = 0.02, $drag = 0.01, $width = 0, $height = 0.2;
	...
	{
		return new EntitySizeInfo(0.2, 0.0, 0.2);
	}
	...
	protected function onHitBlock(Block $blockHit, RayTraceResult $hitResult): void
	{
		if ($this->getOwningEntity()) {
			...
			if (in_array($blockHit->getTypeId(), $blocksAllowed, true)) {
				...
				if ($blockBack->getTypeId() === BlockTypeIds::AIR && $blockBack->getPosition()->getWorld()->getBlock($blockBack->getPosition()->add(0, 1, 0))->getTypeId() === BlockTypeIds::AIR) {
					$this->getWorld()->addSound($this->getOwningEntity()->getPosition(), new EndermanTeleportSound());
