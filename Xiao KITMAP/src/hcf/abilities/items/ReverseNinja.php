...
class ReverseNinja implements Listener
{
    ...
    public function onItemUse(PlayerItemUseEvent $event): void
    {
        ...
        if ($player instanceof Player)
            if ($item->getNamedTag()->getTag("Abilities") !== null) {
                if ($item->getNamedTag()->getString("Abilities") === "ReverseNinja") {
                    $event->cancel();
                    if ($player->getSession()->getCooldown('ability.ReverseNinja') === null) {
                        if ($player->getSession()->getCooldown('ability.global') === null) {
                            ...
                            $player->sendMessage("§6You have activated §bReverse Ninja");
                            ...
                            Loader::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function () use ($damager, $player): void {
                                $damager->teleport($player->getPosition());
                                $player->getSession()->addCooldown('ability.ReverseNinja', ' §l§7×§r §l§bReverse Ninja§r§f: §f', 120);
                        ...
                        $player->sendMessage("§cYou haven't Reverse Ninja in the last " . Timer::format($player->getSession()->getCooldown("ability.ReverseNinja")->getTime()));
                    }
                }
            }
    }
