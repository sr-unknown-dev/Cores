...
class SetCapzoneSubCommand implements KothSubCommand
{
    ...
    public function execute(CommandSender $sender, array $args): void
    {
        ...
        if (count($args) < 1) {
            if (($creator = Loader::getInstance()->getClaimManager()->getCreator($sender->getName())) !== null && $creator->getType() === 'capzone') {
                if (!$creator->isValid()) {
                    $sender->sendMessage(TextFormat::colorize('&cYou have not selected the capzone'));
                    return;
                }
                $creator->deleteCorners($sender);
                $koth = Loader::getInstance()->getKothManager()->getKoth($creator->getName());
        ...
        Loader::getInstance()->getClaimManager()->createCreator($sender->getName(), $name, 'capzone');
