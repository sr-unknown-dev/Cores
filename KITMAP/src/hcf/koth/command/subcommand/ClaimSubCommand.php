...
class ClaimSubCommand implements KothSubCommand
{
    ...
    public function execute(CommandSender $sender, array $args): void
    {
        ...
        if (count($args) < 1) {
            if (($creator = Loader::getInstance()->getClaimManager()->getCreator($sender->getName())) !== null && $creator->getType() === 'koth') {
                if (!$creator->isValid()) {
        ...
        Loader::getInstance()->getClaimManager()->createCreator($sender->getName(), $name, 'koth');
