...
class KothManager
{
    ...
    {
        # Register command
        Loader::getInstance()->getServer()->getCommandMap()->register('HCF', new KothCommand());
        # Register koths
        foreach (Loader::getInstance()->getProvider()->getKoths() as $name => $data)
            $this->createKoth($name, (int)$data['time'], (int) $data['points'], $data['coords'], $data['claim'], $data['capzone']);
            Loader::getInstance()->getScheduler()->scheduleRepeatingTask(new AutoKothTask(), Timer::time("4h") * 20);
    }
     ...
     */
    public function createKoth(string $name, int $time, int $points = 6, ?string $coords = null, ?array $claim = null, ?array $capzone = null): void
    {
