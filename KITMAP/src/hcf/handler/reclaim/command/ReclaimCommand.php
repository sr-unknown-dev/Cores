...
class ReclaimCommand extends Command
{
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void
    {
        ...
        if (!isset($args[0])) {
            $reclaimManager = Loader::getInstance()->getHandlerManager()->getReclaimManager();
            ...
            foreach ($reclaimManager->getReclaims() as $reclaim) {
                if ($sender->hasPermission($reclaim->getPermission())) {
                    if ($sender->getSession()->getCooldown($reclaim->getName() . '.reclaim') === null) {
                        ...
                        Server::getInstance()->broadcastMessage(TextFormat::colorize("&e" . $sender->getName() . " &7has redeemed their §b§l" . $reclaim->getName() . "&r &7rewards using §8(§f/reclaim§8)"));
                    } else {
