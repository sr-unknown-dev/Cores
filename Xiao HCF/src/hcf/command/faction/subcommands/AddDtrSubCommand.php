...
class AddDtrSubCommand extends BaseSubCommand
{
    ...
    protected function prepare(): void
    {
        ...
        $this->registerArgument(0, new RawStringArgument("factionName"));
        $this->registerArgument(1, new RawStringArgument("Dtr"));
    }
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
		...
		if (count($args) < 2) {
			$sender->sendMessage(TextFormat::colorize('&cUse /f adddtr [faction] [dtr]'));
			return;
		}
        ...
        $currentStrikes = $factionInstance->getDtr();

        $newStrikes = $currentStrikes + $dtr;

        $factionInstance->setDtr($newStrikes);
		$sender->sendMessage(TextFormat::colorize('&8[&bAdmin&8] &aYou have updated dtr of ' . $faction . ' faction.'));
    }
