...
namespace hub\utils\inventorie;

use CortexPE\DiscordWebhookAPI\Embed;
use CortexPE\DiscordWebhookAPI\Message;
use CortexPE\DiscordWebhookAPI\Webhook;
use hub\abilities\items\NinjaStar;
use hub\command\moderador\EventsCommand;
use hub\entity\AbilityEntity;
use hub\handler\kit\classes\presets\Archer;
use hub\Loader;
use hub\kits\ArcherOp;
use hub\kits\BardOp;
use hub\kits\Extreme;
use hub\kits\Leviathan;
use hub\kits\RogueOp;
use hub\kits\Supreme;
use hub\kits\Xiao;
use hub\kits\XiaoPlus;
use hub\koth\command\subcommand\StartSubCommand;
use hub\player\Player;
use hub\timer\types\TimerAirdrop;
use hub\timer\types\TimerDeath;
use hub\timer\types\TimerFFA;
use hub\timer\types\TimerKey;
use hub\timer\types\TimerKeyOP;
use hub\timer\types\TimerLoobox;
use hub\timer\types\TimerMystery;
use hub\timer\types\TimerPackages;
use hub\timer\types\TimerPurge;
use hub\timer\types\TimerSotw;
use hub\timer\types\Timerx2Points;
use hub\utils\coldowns\Cooldowns;
    ...
    public static function HCFEvents(Player $player, $time): void
    {
        ...
        $menu->setListener(function (InvMenuTransaction $transaction) use ($time): InvMenuTransactionResult {
            ...
            if ($item === TE::colorize("&aKeyall")) {
                ...
                Loader::getInstance()->getServer()->broadcastMessage(TE::colorize("&7█&a█&7███&a█&7█"));
                Loader::getInstance()->getServer()->broadcastMessage(TE::colorize("&7███████"));
                $webHook = new Webhook(Loader::getInstance()->getConfig()->get('hub.webhook'));
                $msg = new Message();
