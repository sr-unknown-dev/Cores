...
class ClaimCreator extends Claim
{
    ...
    public function setFirstPosition(int $x, int $z): void
    {
        $this->first = ['x' => $x, 'z' => $z];
    }
    public function setSecondPosition(int $x, int $z): void
    {
        $this->second = ['x' => $x, 'z' => $z];
    }
    ...
    public function calculateClaim(Position $first, Position $second): bool
    {
        ...
        for ($x = $minX; $x <= $maxX; $x++) {
            for ($z = $minZ; $z <= $maxZ; $z++) {
                ...
                if (Loader::getInstance()->getClaimManager()->insideClaim($position) !== null)
                    return true;
            }
        }
        return false;
    ...
    public function deleteCorners(Player $player): void
    {
        ...
        if ($first !== null) {
            for ($y = $first->getFloorY(); $y <= 127; $y++) {
                $player->getNetworkSession()->sendDataPacket($this->sendFakeBlock(new Position($first->getFloorX(), $y, $first->getFloorZ(), $first->getWorld()), VanillaBlocks::AIR()));
            }
        }
        ...
        if ($second !== null) {
            for ($y = $second->getFloorY(); $y <= 127; $y++) {
                $player->getNetworkSession()->sendDataPacket($this->sendFakeBlock(new Position($second->getFloorX(), $y, $second->getFloorZ(), $second->getWorld()), VanillaBlocks::AIR()));
            }
        }
    }
    ...
    private function createCorner(Player $player, Position $position, bool $first = true): void
    {
        if ($first) {
            for ($y = $position->getFloorY(); $y <= 127; $y++) {
                $player->getNetworkSession()->sendDataPacket($this->sendFakeBlock(new Position($position->getFloorX(), $y, $position->getFloorZ(), $position->getWorld()), $y % 3 === 0 ? VanillaBlocks::EMERALD() : VanillaBlocks::GLASS()));
            }
        } else {
            $second = $this->getSecond();
            ...
            if ($second !== null && !$second->equals($position)) {
                for ($y = $second->getFloorY(); $y <= 127; $y++) {
                    $player->getNetworkSession()->sendDataPacket($this->sendFakeBlock(new Position($second->getFloorX(), $y, $second->getFloorZ(), $second->getWorld()), VanillaBlocks::STAINED_GLASS()->setColor(DyeColor::RED())));
                }
            }
            for ($y = $position->getFloorY(); $y <= 127; $y++) {
                $player->getNetworkSession()->sendDataPacket($this->sendFakeBlock(new Position($position->getFloorX(), $y, $position->getFloorZ(), $position->getWorld()), $y % 3 === 0 ? VanillaBlocks::EMERALD() : VanillaBlocks::GLASS()));
