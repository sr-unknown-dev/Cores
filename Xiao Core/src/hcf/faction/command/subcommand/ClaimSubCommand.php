...
class ClaimSubCommand implements FactionSubCommand
{
    ...
    public function execute(CommandSender $sender, array $args): void
    {
        ...
        if (count($args) < 1) {
            if (($creator = Loader::getInstance()->getClaimManager()->getCreator($sender->getName())) !== null && $creator->getType() === 'faction') {
                if (!$creator->isValid()) {
        ...
        if (Loader::getInstance()->getClaimManager()->getCreateByClaimName($faction->getName())) {
            $sender->sendMessage(TextFormat::colorize('&cSomeone from your faction is already claim'));
            return;
        }
        
        if (Loader::getInstance()->getClaimManager()->getClaim($faction->getName()) !== null) {
        ...
        Loader::getInstance()->getClaimManager()->createCreator($sender->getName(), $faction->getName(), 'faction');
