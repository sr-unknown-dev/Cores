    ...
    public function handlePlace(BlockPlaceEvent $event): void{
        ...
        if ($item->getNamedTag()->getTag("Airdrop_Item") && $item->getNamedTag()->getString("Airdrop_Item") === "Airdrop"){
            ...
            if ($tile instanceof Chest) {
                ...
                $player->getWorld()->addParticle($TextPosition, $floatingText);

                Loader::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function() use ($TextPosition, $floatingText, $player): void {
                    $player->getWorld()->addParticle($TextPosition, new FloatingTextParticle("", ""));
                }), 120);
                $event->cancel();
            }else{
